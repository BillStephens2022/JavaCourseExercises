...
public class CopyingClasses {
    ...
    public static void main(String[] args) {
        ...
        // shallow copy below: elements share same spot in memory as original
//        Person[] personsCopy = Arrays.copyOf(persons, persons.length);

        // Note that clone method only produces a shallow copy as well
        // Person[] personsCopy = persons.clone();

        // Making a Deep Copy, method 1 & 2
...
//            var kids = current.kids() == null ? null : Arrays.copyOf(current.kids(), current.kids().length);
            // deep copy, there are new instances for each element in the array
            // i.e. doesn't share same spot in memory as the original
            // personsCopy[i] = new Person(current.name(), current.dob(), kids);
            // Method 2, after adding a constructor to Person record, simpler method
            // personsCopy[i] = new Person((persons[i]));


        // }

        // Method 3 - again using new constructor under Person record, but removing the form loop and using lambda expression.
        Person[] personsCopy = new Person[5];
